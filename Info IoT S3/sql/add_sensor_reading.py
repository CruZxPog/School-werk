...
def read_user_input():
    ...
    reading_value = input("Enter reading value: ").strip()
    if not reading_value:
        print("Input error: Reading value cannot be empty.")
        sys.exit(1) # Input error

    if len(reading_value) > 255:
        print("Input error: Reading value exceeds 255 characters.")
        sys.exit(1) # Input error
    return serial_number, sensor_type, reading_value
...
def get_sensor_id(db_cursor, serial_number, sensor_type):
    # Check if sensor exists
        ...
        if len(result) == 0:
            ...
            print(f"Input error: Multiple sensors found with serial number '{serial_number}' and type '{sensor_type}'.")
            sys.exit(1) # Input error
        return result[0][0]

def main():
    try:
        # sensor_id = read_integer_input("Enter sensor ID: ")
        serial_number, sensor_type, reading_value = read_user_input()
        ...
        val = (sensor_id, reading_value)
        db_cursor.execute(sql, val)
        db_connection.commit()

        print(f"Success: Inserted reading ID = {db_cursor.lastrowid} for sensor {sensor_id}.")
